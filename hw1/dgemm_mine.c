...
*/
void square_dgemm3(const int M, 
                  const double *A, const double *B, double *C)
{
    int i, j, k;
    for (i = 0; i < M; ++i) {
...
void square_dgemm(const int M, const double *A, const double *B, double *C)
{
  ...
  for (panel = 0; panel < num_A_blocks; ++panel) {
    ...
    const int panel_width = CALC_CUR_BLOCK_WIDTH(panel_pos, 
						 A_BLOCK_LEN, 
						 M);
	...
	*/
    ...
    for (C_block = 0; C_block < num_C_blocks; ++C_block) {
      ...
      const int C_block_width = CALC_CUR_BLOCK_WIDTH(C_block_pos,
						     C_BLOCK_LEN, 
						     M);	     
      ...
      for(int j = 0; j < panel_width; ++j)
	{
	  ...
	  */
	  for(int i = 0; i < C_block_width; ++i)
	    {
	      A_pack[i+j*C_BLOCK_LEN] = A[(C_block_pos+i)+(panel_pos+j)*M];
	    }
	}
	...
	to_kdgemm_B_sized(A_BLOCK_LEN, 
			  B_pack + P_block_pos * A_BLOCK_LEN,
			  B_kernel, 
			  panel_width, 
			  P_block_width);
	for (K_block = 0; K_block < num_K_blocks; ++K_block) {
	  ...
	  pmat(C_kernel,2,2);
	  from_kdgemm_C_sized(K_BLOCK_LEN, 
			      C_kernel, 
			      C_pack + K_block_pos,
			      K_block_width, 
			      P_block_width);
	       ...
	       const int num_cols)
{
  for(int col = 0; col < num_cols; ++col)
    {
      memcpy(dest + A_BLOCK_LEN * col,
	     source + A_BLOCK_LEN * col,
	     num_rows * sizeof(double));
    }
}

void to_A_pack(double* dest, 
	       double* source, 
	       const int num_rows, 
	       const int num_cols)
{
  for(int col = 0; col < num_cols; ++col)
    {
      memcpy(dest + C_BLOCK_LEN * col, 
	     source + C_BLOCK_LEN * col, 
	     num_rows * sizeof(double));
    }
}

void from_C_pack(double* C, 
		 double* C_pack, 
		 const int num_rows, 
